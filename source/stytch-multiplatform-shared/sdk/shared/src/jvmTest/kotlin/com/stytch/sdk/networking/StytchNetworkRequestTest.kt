...
internal class StytchNetworkRequestTest {
    ...
    @Test
    fun `returns data and calls onSuccess on success`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `calls onError and throws the result on ResponseException`() =
        runTest(dispatcher) {
            val apiError =
                StytchAPIError(
                    statusCode = 400,
            ...
            val thrown =
                assertFailsWith<StytchAPIError> {
    ...
    @Test
    fun `wraps unknown exceptions in StytchNetworkError without calling onError`() =
        runTest(dispatcher) {
            val cause = RuntimeException("unexpected")
            val thrown =
                assertFailsWith<StytchNetworkError> {
...
internal class StytchNetworkRequestWithRetryAndBackoffTest {
    ...
    @Test
    fun `returns result immediately on first success`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `retries on failure and returns result on eventual success`() =
        runTest(dispatcher) {
            var callCount = 0
            val result =
    ...
    @Test
    fun `rethrows after max retries exhausted`() =
        runTest(dispatcher) {
            var callCount = 0
