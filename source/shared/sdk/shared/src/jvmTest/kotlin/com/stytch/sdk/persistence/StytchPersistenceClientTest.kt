...
class StytchPersistenceClientTest {
    ...
    @Test
    fun `save writes encrypted base64-encoded value to platform client`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `save with null removes the key`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `get returns default when key is absent`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `get returns default and removes key when decrypted bytes are not valid JSON`() =
        runTest(dispatcher) {
            every { encryptionClient.encrypt(any()) } answers { firstArg() }
    ...
    @Test
    fun `remove delegates to platform client`() =
        runTest(dispatcher) {
            client.remove("key")
    ...
    @Test
    fun `save then get returns original value`() =
        runTest(dispatcher) {
            every { encryptionClient.encrypt(any()) } answers { firstArg() }
    ...
    @Test
    fun `save null after save removes key so get returns default`() =
        runTest(dispatcher) {
            every { encryptionClient.encrypt(any()) } answers { firstArg() }
            every { encryptionClient.decrypt(any()) } answers { firstArg() }
