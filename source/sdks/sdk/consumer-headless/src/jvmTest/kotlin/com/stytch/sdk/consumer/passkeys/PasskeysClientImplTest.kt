...
internal class PasskeysClientImplTest : ConsumerClientTest() {
    ...
    @Test
    fun `register throws PasskeysUnsupportedError when not supported`() =
    ...
    @Test
    fun `register calls registerStart, threads options to provider, then calls webAuthnRegister`() =
        runTest(testDispatcher) {
            every { passkeyProvider.isSupported } returns true
            val c = PasskeysClientImpl(dispatchers, networkingClient, sessionManager, passkeyProvider)
    ...
    @Test
    fun `authenticate throws PasskeysUnsupportedError when not supported`() =
        runTest(testDispatcher) {
            every { passkeyProvider.isSupported } returns false
            val c = PasskeysClientImpl(dispatchers, networkingClient, sessionManager, passkeyProvider)
    ...
    @Test
    fun `authenticate calls primary start and threads options to provider when no session token`() =
        runTest(testDispatcher) {
            every { passkeyProvider.isSupported } returns true
    ...
    @Test
    fun `update throws PasskeysUnsupportedError when not supported`() =
        runTest(testDispatcher) {
            every { passkeyProvider.isSupported } returns false
            val c = PasskeysClientImpl(dispatchers, networkingClient, sessionManager, passkeyProvider)
    ...
    @Test
    fun `update calls webAuthnUpdate with id and correct network model`() =
        runTest(testDispatcher) {
            every { passkeyProvider.isSupported } returns true
            val c = PasskeysClientImpl(dispatchers, networkingClient, sessionManager, passkeyProvider)
