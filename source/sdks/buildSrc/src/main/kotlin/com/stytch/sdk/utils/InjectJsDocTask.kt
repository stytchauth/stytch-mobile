...
/**
 * Reads a KSP-generated JsDoc map and injects JSDoc comments into a KMP-generated
 * TypeScript declarations file (.d.mts).
 *
 * KMP silently drops KDocs from JS output. This task restores them by:
 *   1. Reading the map produced by StytchJsDocExtractor (keys like "TypeName",
...
abstract class InjectJsDocTask : DefaultTask() {
    ...
    fun inject() {
        ...
        if (!mapFile.exists()) {
            logger.warn("InjectJsDocTask: jsdoc-map.txt not found at ${mapFile.absolutePath}, skipping")
            return
        }
        if (!dts.exists()) {
        ...
        val added =
            injected.lines().count { it.trimStart().startsWith("/**") } -
