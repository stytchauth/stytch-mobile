...
internal class OAuthClientImplTest : ConsumerClientTest() {
    ...
    @Test
    fun `authenticate retrieves PKCE and calls oAuthAuthenticate with code verifier`() =
    ...
    @Test
    fun `authenticateGoogleIdToken calls oAuthGoogleIDTokenAuthenticate with correct model`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `authenticateAppleIdToken calls oAuthAppleIDTokenAuthenticate with correct model`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `attach calls oAuthAttach with correct model`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `start uses CNAME domain when provided`() =
        runTest(testDispatcher) {
            var capturedUrl = ""
            coEvery { oauthProvider.getOAuthToken(any(), any(), any(), any(), any(), any()) } coAnswers {
                capturedUrl = arg(4)
    ...
    @Test
    fun `start uses live domain when no CNAME and token is a live token`() =
        runTest(testDispatcher) {
            var capturedUrl = ""
            coEvery { oauthProvider.getOAuthToken(any(), any(), any(), any(), any(), any()) } coAnswers {
                capturedUrl = arg(4)
    ...
    @Test
    fun `start with ClassicToken calls authenticate`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `start with Google IDToken calls authenticateGoogleIdToken`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `start with non-Google IDToken calls authenticateAppleIdToken`() =
        runTest(testDispatcher) {
            coEvery { oauthProvider.getOAuthToken(any(), any(), any(), any(), any(), any()) } returns
                OAuthResult.IDToken(token = "id-tok", nonce = "nonce123")
            coEvery { api.oAuthAppleIDTokenAuthenticate(any()) } returns StytchDataResponse(mockk(relaxed = true))
    ...
    @Test
    fun `start uses sessionDurationMinutes from parameters when provided`() =
        runTest(testDispatcher) {
            coEvery { oauthProvider.getOAuthToken(any(), any(), any(), any(), any(), any()) } returns OAuthResult.ClassicToken("tok")
            coEvery { pkceClient.retrieve() } returns fakePair
            coEvery { api.oAuthAuthenticate(any()) } returns StytchDataResponse(mockk(relaxed = true))
    ...
    @Test
    fun `name with 3 or more parts parses to firstName, middleName, and joined lastName`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `name with 2 parts parses to firstName and lastName`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `name with 1 part parses to firstName only`() =
        runTest(testDispatcher) {
