...
class StytchConsumerAuthenticationStateManagerTest {
    ...
    private val fakeUser = ApiUserV1User(
        userId = "user-id",
        emails = emptyList(),
        status = "active",
    ...
    private val fakeSession = ApiSessionV1Session(
    ...
    private val fakeResponse = object : AuthenticatedResponse {
    ...
    @Test
    fun `authenticationStateFlow emits Loading before init completes`() = runTest(testDispatcher) {
    ...
    @Test
    fun `authenticationStateFlow emits Unauthenticated after init with no persisted data`() = runTest(testDispatcher) {
    ...
    @Test
    fun `authenticationStateFlow emits Authenticated when all four values are set`() = runTest(testDispatcher) {
        val manager = createManager()
        val states = mutableListOf<ConsumerAuthenticationState>()
        val job = launch { manager.authenticationStateFlow.collect { states.add(it) } }
        advanceUntilIdle()
    ...
    @Test
    fun `authenticationStateFlow emits Unauthenticated after revoke`() = runTest(testDispatcher) {
        val manager = createManager()
        manager.update(fakeResponse)
        advanceUntilIdle()
    ...
    @Test
    fun `update with AuthenticatedResponse sets all four flow values`() = runTest(testDispatcher) {
        val manager = createManager()
        manager.update(fakeResponse)
    ...
    @Test
    fun `update with AuthenticatedResponse persists all four values`() = runTest(testDispatcher) {
        val manager = createManager()
        manager.update(fakeResponse)
        advanceUntilIdle()
    ...
    @Test
    fun `update with non-AuthenticatedResponse does not modify flows`() = runTest(testDispatcher) {
        val manager = createManager()
    ...
    @Test
    fun `revoke clears all four flow values`() = runTest(testDispatcher) {
        val manager = createManager()
        manager.update(fakeResponse)
        manager.revoke()
    ...
    @Test
    fun `revoke removes all four keys from persistence`() = runTest(testDispatcher) {
        val manager = createManager()
        manager.update(fakeResponse)
        advanceUntilIdle()
        manager.revoke()
    ...
    @Test
    fun `currentSessionToken reflects sessionTokenFlow value`() = runTest(testDispatcher) {
        val manager = createManager()
    ...
    @Test
    fun `currentSessionToken is null after revoke`() = runTest(testDispatcher) {
        val manager = createManager()
        manager.update(fakeResponse)
        manager.revoke()
    ...
    @Test
    fun `init loads all four values from persistence and emits Authenticated`() = runTest(testDispatcher) {
